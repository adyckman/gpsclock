...
### `timezone.py`
Defines 7 US timezones with automatic DST support. Computes DST transitions (2nd Sunday of March, 1st Sunday of November) from the GPS date and adjusts offset and abbreviation accordingly. Arizona and Hawaii are marked as non-DST. GPIO14 button with pull-up and 250ms debounce cycles through zones on each press. On first GPS fix, auto-detects the timezone from coordinates using `tz_grid.py`; manual button presses take priority over auto-detection.
